...
# (comment) `lubridate` to work with Time in R
...
# Prepare the Temperature dataset for time-series analysis
...
We can plot the temperature of a single day, for instance the 1st January 2020:
...
## Create a new time object considering local time

Data are in UTC! So we want to change time-zone to the Pacific Time Zone (PT): 
...
temperature_df$TIME_LOCAL[duplicated(as.character(temperature_df$TIME_LOCAL))] %>% knitr::kable()
```

Why we are checking `as.character()`? There are duplicated values at the same months... every year... the same hour! 

Indeed.. on local DST there are one missing hour and one duplicated hour every year:
...
We might want to have a Solar Local Time. This make more sense when making some Climatic 
analysis.

## Create a time-series in Local Solar Time
